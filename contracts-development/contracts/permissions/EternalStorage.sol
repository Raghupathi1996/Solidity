...
/** @title Eternal Storage contract
  ...
  * Some contracts/accounts will be given access via the PermissionsUpgradable.sol contract
...
contract EternalStorage {
    ...
    /** @notice Modifier used for updateContractDatabaseAccess() method.
      * So that only PermissionsUpgradable.sol is allowed to grant and revoke access.
      */
    ...
    onlyUpgradeable {
        ...
        if(!isKeyExists[storageKey]){
            uint id = boolKeys.length++;
            boolKeys[id] = storageKey;
    ...
    /** @notice changes the upgradable contract address to the new address.
      ...
      */
    function changePermUpgradable(address _proposedUpgradable) public 
    onlyUpgradeable {
      permImplUpgradeable = _proposedUpgradable;
    }
