...
/** @title Permissions Implementation Contract
  * @notice This contract holds implementation logic for all permissions
    ...
    contract.
  */
contract PermissionsImplementation is NodeStructs, RoleStructs, OrgStructs {
    ...
    /** @notice function to create a sub org under a given parent org, create a new role for this org,
        assign an account for this org with the created role, assign name to the assigned account.
      * @param _pOrgId parent org id under which the sub org is being added
      * @param _orgId unique id for the sub organization
      * @param _nodeDetails Node details object containing orgId, enodeId, ip, port, raftPort and status. We can provide arbitrary orgId and status
        since these values are assigned automatically
      * @param _roleDetails object containing roleId, orgId, baseAccess, isVoter, isAdmin, active. We can provide arbitrary values for orgId and active
        since these values are assigned automatically
      * @param _account Account address to be assigned to this org
      * @param _accountName Blockchain username of the account
      * @param _caller Passed by permission interface to check if caller is admin of parent org
      ...
      */
    function addSubOrgWithAccountAndRole(string memory _pOrgId, string memory _orgId, 
        bytes memory _nodeDetails, bytes memory _roleDetails, bytes memory _orgExtDetails, 
        address _account, string memory _accountName, address _caller) public 
    onlyInterface
    orgExists(_pOrgId) orgAdmin(_caller, _pOrgId) {
        RoleDetails memory roleDetails = abiDecodeRoleDetails(_roleDetails);
        orgManager.addSubOrg(_pOrgId, _orgId);
        string memory fullOrgId = string(abi.encodePacked(_pOrgId, ".", _orgId));
        if(_orgExtDetails.length>0){
          OrgExternalDetails memory orgExtDetails = abiDecodeOrgExtDetails(_orgExtDetails);
          orgManager.updateOrgExternalDetails(fullOrgId, orgExtDetails.name, orgExtDetails.location, orgExtDetails.url);
        }
        if (_nodeDetails.length > 0) {
          NodeDetails memory nodeDetails = abiDecodeNodeDetails(_nodeDetails);
            nodeManager.addOrgNode(nodeDetails.enodeId, nodeDetails.ip, nodeDetails.port, nodeDetails.raftPort, fullOrgId);
        }
        roleManager.addRole(roleDetails.roleId, fullOrgId, roleDetails.baseAccess, roleDetails.isVoter, roleDetails.isAdmin);
        accountManager.assignAccountRole(_account, fullOrgId, roleDetails.roleId, roleDetails.isAdmin);
        _assignAccountName(_account, fullOrgId, _accountName, _caller);
    }
    ...
    function updateOrgExternalDetails(string calldata _orgId, string calldata _name, string calldata _location, string calldata _url, address _caller) external onlyInterface
    orgExists(_orgId) orgAdmin(_caller, _orgId) {
        orgManager.updateOrgExternalDetails(_orgId, _name, _location, _url);
    }
    
    function updateOrgUrl(string calldata _orgId, string calldata _url, address _caller) external onlyInterface
    orgExists(_orgId) orgAdmin(_caller, _orgId) {
        orgManager.updateOrgUrl(_orgId, _url);
    }
    ...
    function assignAccountName(address _account,
        string memory _name, address _caller) public
    onlyInterface {
        (string memory orgId, ) = accountManager.getAccountOrgRole(_account);
        _assignAccountName(_account, orgId, _name, _caller);
    }
    ...
    /** @notice function to assigns a role id to the account given account
      ...
      * @param _roleId role id to be assigned to the account
      */
    function assignAccountRoleAndName(address _account, string memory _orgId,
        string memory _roleId, string memory _name, address _caller) public
    onlyInterface
    orgAdmin(_caller, _orgId)
    orgApproved(_orgId) {
        require(validateAccount(_account, _orgId) == true, "operation cannot be performed");
        ...
        bool admin = roleManager.isAdminRole(_roleId, _orgId, _getUltimateParent(_orgId));
        accountManager.assignAccountRole(_account, _orgId, _roleId, admin);
        _assignAccountName(_account, _orgId, _name, _caller);
    }
    ...
    function validateOrgAndAccount(address _account, string memory _orgId) public view
    returns (bool){
        if(!_checkOrgExists(_orgId)){
          return false;
        }
        return (accountManager.validateAccountAssigned(_account, _orgId));
    }
    ...
    function _assignAccountName(address _account, string memory _orgId,
        string memory _name, address _caller) internal
    orgAdmin(_caller, _orgId)
    orgApproved(_orgId) {
        require(validateOrgAndAccount(_account, _orgId) == true, "operation cannot be performed");
        accountManager.assignAccountName(_account, _name);
    }
