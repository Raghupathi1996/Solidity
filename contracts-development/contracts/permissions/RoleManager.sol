...
contract RoleManager {
    ...
    /** @notice constructor. sets the permissions upgradable address
      */
    constructor (address _permUpgradable, address _roleModel) public {
        permUpgradable = PermissionsUpgradable(_permUpgradable);
        roleModel = RoleModel(_roleModel);
    }
    ...
    function addRole(string memory _roleId, string memory _orgId, uint256 _baseAccess,
        ...
        require(roleModel.getRoleIndex(keccak256(abi.encode(_roleId, _orgId))) == 0, "role exists for the org");
        roleModel.incNumberOfRoles();
        roleModel.setRoleIndex(keccak256(abi.encode(_roleId, _orgId)),roleModel.getNumberOfRoles());
        roleModel.pushRole(_roleId, _orgId, _baseAccess, _isVoter, _isAdmin, true);
    ...
    onlyImplementation {
        require(roleModel.getRoleIndex(keccak256(abi.encode(_roleId, _orgId))) != 0, "role does not exist");
        uint256 rIndex = _getRoleIndex(_roleId, _orgId);
        roleModel.setRoleActive(rIndex,false);
    ...
    function isVoterRole(string calldata _roleId, string calldata _orgId,
        ...
        uint256 rIndex;
        if (roleModel.getRoleIndex(keccak256(abi.encode(_roleId, _orgId))) != 0) {
            rIndex = _getRoleIndex(_roleId, _orgId);
        }
        else {
            rIndex = _getRoleIndex(_roleId, _ultParent);
        }
        return (roleModel.getRoleActive(rIndex) && roleModel.getRoleIsVoter(rIndex));
    }
    ...
    function isAdminRole(string calldata _roleId, string calldata _orgId,
        ...
        return (roleModel.getRoleActive(rIndex) && roleModel.getRoleIsAdmin(rIndex));
    }
    ...
    external view returns (string memory roleId, string memory orgId,
        ...
        uint256 rIndex = _getRoleIndex(_roleId, _orgId);
        return (roleModel.getRoleRoleId(rIndex), roleModel.getRoleOrgId(rIndex),
        roleModel.getRoleBaseAccess(rIndex), roleModel.getRoleIsVoter(rIndex),
        roleModel.getRoleIsAdmin(rIndex), roleModel.getRoleActive(rIndex));
    }
    ...
    (string memory roleId, string memory orgId, uint256 accessType,
        ...
        return (roleModel.getRoleRoleId(_rIndex), roleModel.getRoleOrgId(_rIndex),
        roleModel.getRoleBaseAccess(_rIndex), roleModel.getRoleIsVoter(_rIndex),
        roleModel.getRoleIsAdmin(_rIndex), roleModel.getRoleActive(_rIndex));
    }
    ...
    function getNumberOfRoles() external view returns (uint256) {
        return roleModel.getNumberOfRoles();
    }
    ...
    function roleExists(string memory _roleId, string memory _orgId,
        ...
        if (roleModel.getRoleIndex(keccak256(abi.encode(_roleId, _orgId))) != 0) {
            ...
            return roleModel.getRoleActive(id);
        }
        else if (roleModel.getRoleIndex(keccak256(abi.encode(_roleId, _ultParent))) != 0) {
    ...
    function roleAccess(string memory _roleId, string memory _orgId,
        ...
        if (roleModel.getRoleIndex(keccak256(abi.encode(_roleId, _orgId))) != 0) {
            id = _getRoleIndex(_roleId, _orgId);
            return roleModel.getRoleBaseAccess(id);
        }
        else if (roleModel.getRoleIndex(keccak256(abi.encode(_roleId, _ultParent))) != 0) {
            id = _getRoleIndex(_roleId, _ultParent);
            return roleModel.getRoleBaseAccess(id);
    ...
    function transactionAllowed(string calldata _roleId, string calldata _orgId,
        ...
        if (_typeOfTxn == 3 && (access == 1 || access == 5 || access == 6)){
            return true;
        }
        if (_typeOfTxn == 2 && (access == 2 || access == 6 || access == 7)){
            return true;
        }
        if (_typeOfTxn == 1 && (access == 4 || access == 5 || access == 7)){
            return true;
        }
    ...
    internal view returns (uint256) {
        return roleModel.getRoleIndex(keccak256(abi.encode(_roleId, _orgId))) - 1;
    }
}
