...
contract BankGuaranteeManager is BankGuaranteeStructs {
    ...
    event BankGuaranteeRegistered(bytes8 _bgUid, string _bgReferenceNumber, bytes8 _paUid, string _paymentAdviceReferenceNumber, string _egpSystemId);

    event AmendedBankGuaranteeRegistered(bytes8 _bgUid, string _bgReferenceNumber, string paymentAdviceNumber, uint _bgAmendment, string _egpSystemId);
    ...
    modifier validatePaymentAdvice(BankGuarantee memory _bankGuarantee) {
        ...
        require(bankGuaranteeModel.getBankGuaranteeIndexListLengthByPaymentAdviceAndBankGid(
            bytes8(keccak256(abi.encodePacked(_bankGuarantee.paymentAdviceNumber, _bankGuarantee.bankGid)))) == 0, "BG number already exists in the system" );
        _;
    }
    ...
    modifier checkPaReferenceNumberByBankGid (BankGuarantee memory _bankGuarantee) {
        require(bankGuaranteeModel.getBankGuaranteeIndexListLengthByPaymentAdviceAndBankGid(
            bytes8(keccak256(abi.encodePacked(_bankGuarantee.paymentAdviceNumber, _bankGuarantee.bankGid)))) > 0, "Initial BG is not been submitted or BG Ref Number not found in the system to submit amendment");
        require(paymentAdviceManager.getLatestPaUidByReferenceNumber(_bankGuarantee.paymentAdviceNumber) == _bankGuarantee.paymentAdviceUid, "Submit the BG against the latest PA pushed by the Egp system");
        require(bankGuaranteeModel.getBankGuaranteforPaymentAdviceBankGid(_bankGuarantee.bankGid, _bankGuarantee.paymentAdviceUid) == 0, "Bank has already submitted BG for the PA");
        require(vcnFunctionalManager.checkCurrencyVersion(paymentAdviceManager.getPaymentAdviceCurrencyType(_bankGuarantee.paymentAdviceUid), _bankGuarantee.currency), "Currency doesn't match as per the Payment Advice");
        _;
    }
    ...
    validatePaymentAdvice(_bankGuarantee)
    {
        ...
        emit BankGuaranteeRegistered(_bankGuarantee.uid, _bankGuarantee.referenceNumber, _bankGuarantee.paymentAdviceUid, _bankGuarantee.paymentAdviceNumber, paymentAdviceManager.getPaymentAdviceEgpbyUid(_bankGuarantee.paymentAdviceUid));
    }
    ...
    function _publishBankGuarantee(BankGuarantee memory _bankGuarantee) internal
    {
        ...
        // );
        _setBankGuaranteeIndices(_bankGuarantee, id);
        // string memory _vendorGid = paymentAdviceManager.getPaymentAdviceVendorGidbyUid(_bankGuarantee.paymentAdviceUid);
        // string memory _peGid = paymentAdviceManager.getPaymentAdviceProcuringEntityGidbyUid(_bankGuarantee.paymentAdviceUid);
    ...
    onlyBankRepresentative(_bankGuarantee.bankGid)
    checkPaReferenceNumberByBankGid (_bankGuarantee)
    {
        uint _PrevIndex;
        ...
        _bankGuarantee.uid = uid;
        _amendedBankGuarantee(_bankGuarantee, _PrevIndex);
        emit AmendedBankGuaranteeRegistered(_bankGuarantee.uid, _bankGuarantee.referenceNumber, 
        _bankGuarantee.paymentAdviceNumber, _bankGuarantee.amendment,  paymentAdviceManager.getPaymentAdviceEgpbyUid(_bankGuarantee.paymentAdviceUid));  
    }

    function _amendedBankGuarantee(BankGuarantee memory _bankGuarantee, uint _PrevIndex) internal
    {
        uint id = bankGuaranteeModel.getBankGuaranteeListLength();
        bankGuaranteeModel.setBankGuaranteeIndex(_bankGuarantee.uid, 1+id);
        bankGuaranteeModel.incBankGuaranteeListLength();
    ...
    function getBankGuaranteesByPaymentAdviceRefAndEgpId(string memory _paRef, string memory _egpSystemId, uint _toExcluded, uint _count) public view 
    returns(BankGuarantee[] memory bankGuarantees_, uint fromIncluded_, uint length_){
        bytes8 _bgPaRefEgpId = bytes8(keccak256(abi.encodePacked(_paRef, _egpSystemId)));
        uint[] memory indexList;
        (indexList, fromIncluded_, length_) = bankGuaranteeModel.getBankGuaranteeByPaymentAdviceRefAndEgpId(_bgPaRefEgpId, _toExcluded, _count);
        bankGuarantees_ = new BankGuarantee[](indexList.length);
        for (uint256 index = 0; index < indexList.length; index++) {
            bankGuarantees_[index] = bankGuaranteeModel.getBankGuaranteeByIndex(indexList[index]);
        }
    ...
    // struct ReturnBankGuarnatee {
    //     uint _sizeOfMapping;
    //     mapping (uint256 => BankGuarantee) _bankGuarantee;
    // }
    
    // ReturnBankGuarnatee returnBankGuarnatee;

    // event bankGuaranteeEvent(BankGuarantee[] _bankGuarantee, uint _fromIncluded, uint _length);



    // function getBankGuaranteesByVendorGid(string memory _vendorGid, uint _toExcluded, uint _count) public
    // returns(BankGuarantee[] memory bankGuarantees_, uint fromIncluded_, uint length_){
    //     bytes8[] memory paymentAdviceUidList;
    ...
    //     // returnBankGuarnatee storage _byvendorGid;
    //     returnBankGuarnatee._sizeOfMapping = 0;
    //     for(uint i=0;i<paymentAdviceUidList.length;i++) {
    //         if(isBgExistsForPaymentAdviceUid(paymentAdviceUidList[i])){
    //             returnBankGuarnatee._bankGuarantee[returnBankGuarnatee._sizeOfMapping++] = _getBankGuaranteeByPaymentAdviceUid(paymentAdviceUidList[i]);
    //             // bankGuarantees_[i] =  _getBankGuaranteeByPaymentAdviceUid(paymentAdviceUidList[i]);
    //         }
    //     }
    //     bankGuarantees_ = new BankGuarantee[](returnBankGuarnatee._sizeOfMapping);
    //     for(uint i = 0; i < returnBankGuarnatee._sizeOfMapping; i++) {
    //         bankGuarantees_[i] = returnBankGuarnatee._bankGuarantee[i];
    //     }
    //     emit bankGuaranteeEvent(bankGuarantees_, fromIncluded_, length_);
    // } 
    ...
    function getBankGuaranteesByVendorGid(string memory _vendorGid,  uint _toExcluded, uint _count) public view 
      returns(BankGuarantee[] memory _bankGuaranteeByVendorGid, uint fromIncluded_, uint length_)  {
        bytes8 _bgByVendorGid = bytes8(keccak256(abi.encodePacked(_vendorGid)));
        uint[] memory indexList;
        (indexList, fromIncluded_, length_) = bankGuaranteeModel.getBankGuaranteeByVendorGid(_bgByVendorGid, _toExcluded, _count);
        _bankGuaranteeByVendorGid = new BankGuarantee[] (indexList.length);
        for (uint256 i = 0; i < indexList.length; i++) {
            _bankGuaranteeByVendorGid[i] = bankGuaranteeModel.getBankGuaranteeByIndex(indexList[i]);
        }

    }

    function getBankGuaranteesByEgpId(string memory _egpSystemId,  uint _toExcluded, uint _count) public view 
      returns(BankGuarantee[] memory _bankGuaranteeByEgpId, uint fromIncluded_, uint length_)  {
        bytes8 _bgByEgpSystemId = bytes8(keccak256(abi.encodePacked(_egpSystemId)));
        uint[] memory indexList;
        (indexList, fromIncluded_, length_) = bankGuaranteeModel.getBankGuaranteeByEgpId(_bgByEgpSystemId, _toExcluded, _count);
        _bankGuaranteeByEgpId = new BankGuarantee[] (indexList.length);
        for (uint256 i = 0; i < indexList.length; i++) {
            _bankGuaranteeByEgpId[i] = bankGuaranteeModel.getBankGuaranteeByIndex(indexList[i]);
        }
    ...
    function getBankGuaranteesByPeGid(string memory _peGid,  uint _toExcluded, uint _count) public view 
      returns(BankGuarantee[] memory _bankGuaranteeByEgpId, uint fromIncluded_, uint length_)  {
        bytes8 _bgByPeGid = bytes8(keccak256(abi.encodePacked(_peGid)));
        uint[] memory indexList;
        (indexList, fromIncluded_, length_) = bankGuaranteeModel.getBankGuaranteeByPeGid(_bgByPeGid, _toExcluded, _count);
        _bankGuaranteeByEgpId = new BankGuarantee[] (indexList.length);
        for (uint256 i = 0; i < indexList.length; i++) {
            _bankGuaranteeByEgpId[i] = bankGuaranteeModel.getBankGuaranteeByIndex(indexList[i]);
        }
    ...
    function _setBankGuaranteeIndices(BankGuarantee memory _bankGuarantee, uint _id) internal
    {
        bankGuaranteeModel.setBankGuaranteeIndexByPaymentAdviceUid(_bankGuarantee.paymentAdviceUid, 1+_id);
        bankGuaranteeModel.addBankGuaranteforPaymentAdviceBankGid(_bankGuarantee.paymentAdviceUid, _bankGuarantee.bankGid);
        string memory _vendorGid = paymentAdviceManager.getPaymentAdviceVendorGidbyUid(_bankGuarantee.paymentAdviceUid);
        string memory _peGid = paymentAdviceManager.getPaymentAdviceProcuringEntityGidbyUid(_bankGuarantee.paymentAdviceUid);
        string memory _egpSystemId = paymentAdviceManager.getPaymentAdviceEgpbyUid(_bankGuarantee.paymentAdviceUid);
        bankGuaranteeModel.setBankGuaranteeIndices(_bankGuarantee, _id, _vendorGid, _peGid, _egpSystemId);
    } 
