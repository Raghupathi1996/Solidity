...
import "./storage/MemberStorage.sol";
import "./storage/ListStorage.sol";
import "./storage/MapStorage.sol";
...
contract BankGuaranteeModel is BankGuaranteeStructs {
    ...
    string constant internal bankGuaranteeIndexListByPaymentAdviceRefAndEgpId = "map_bankGuaranteeIndexListByPaymentAdviceRefAndEgpId";
    string constant internal bankGuaranteeByVendorGid = "map_bankGuaranteeByVendorGid";
    string constant internal bankGuaranteeByEgpId = "map_bankGuaranteeByEgpId";
    string constant internal bankGuaranteeByPeGid = "map_bankGuaranteeByPeGid";
    ...
    /** @dev Listing out the Bank Guarantees against the Reference Number and Bank Gid.
      ...
      */
    function getBankGuaranteeByBgReferenceAndBankGid(bytes8 _bgRefBankGid,  uint _toExcluded, uint _count) public view 
    returns(uint[] memory _bankGuaranteeByBgReferenceAndBankGid, uint fromIncluded_, uint length_)  {
    ...
    /** @dev Listing out the Bank Guarantees against the Vendor Gid and Bank Gid.
      ...
      */
    function getBankGuaranteeByVendorAndBankGid(bytes8 _bgVendorBankGid,  uint _toExcluded, uint _count) public view 
    returns(uint[] memory _bankGuaranteeByVendorAndBankGid, uint fromIncluded_, uint length_)  {
    ...
    function getBankGuaranteeByVendorGid(bytes8 _bgVendorGid,  uint _toExcluded, uint _count) public view 
    returns(uint[] memory _bankGuaranteeIndexByVendorGid, uint fromIncluded_, uint length_)  {
        //bytes8 _bgVendorBankGid = bytes8(keccak256(abi.encodePacked(_vendorGid, _bankGid)));
        uint length = Map.getByKeyBytes8ToUint(KEY, bankGuaranteeByVendorGid, _bgVendorGid, abi.encodePacked(keyListLength));
        if (_toExcluded > length) {
            _toExcluded = length;
        }
        uint itr = _count>_toExcluded ? _toExcluded:_count;
        _bankGuaranteeIndexByVendorGid = new uint[](itr);
        while (itr>0) {
            itr--;
            _toExcluded--;
            _bankGuaranteeIndexByVendorGid[_bankGuaranteeIndexByVendorGid.length-itr-1] = Map.getByKeyBytes8ToUint(KEY, bankGuaranteeByVendorGid, _bgVendorGid, abi.encodePacked(_toExcluded));
        }
        fromIncluded_ = _toExcluded;
        length_ = length;
    }

    function getBankGuaranteeByPaymentAdviceRefAndEgpId(bytes8 _bgPaRefEgpId,  uint _toExcluded, uint _count) public view 
    returns(uint[] memory _bankGuaranteeIndexByPaymentAdviceRefAndEgpId, uint fromIncluded_, uint length_)  { 
        //bytes8 _bgVendorBankGid = bytes8(keccak256(abi.encodePacked(_vendorGid, _bankGid)));
        uint length = Map.getByKeyBytes8ToUint(KEY, bankGuaranteeIndexListByPaymentAdviceRefAndEgpId, _bgPaRefEgpId, abi.encodePacked(keyListLength));
        if (_toExcluded > length) {
            _toExcluded = length;
        }
        uint itr = _count>_toExcluded ? _toExcluded:_count;
        _bankGuaranteeIndexByPaymentAdviceRefAndEgpId = new uint[](itr);
        while (itr>0) {
            itr--;
            _toExcluded--;
            _bankGuaranteeIndexByPaymentAdviceRefAndEgpId[_bankGuaranteeIndexByPaymentAdviceRefAndEgpId.length-itr-1] = Map.getByKeyBytes8ToUint(KEY, bankGuaranteeIndexListByPaymentAdviceRefAndEgpId, _bgPaRefEgpId, abi.encodePacked(_toExcluded));
        }
        fromIncluded_ = _toExcluded;
        length_ = length;
    }
    ...
    function getBankGuaranteeByEgpId(bytes8 _bgVendorGid,  uint _toExcluded, uint _count) public view 
    returns(uint[] memory _bankGuaranteeByEgpId, uint fromIncluded_, uint length_)  {
        //bytes8 _bgVendorBankGid = bytes8(keccak256(abi.encodePacked(_vendorGid, _bankGid)));
        uint length = Map.getByKeyBytes8ToUint(KEY, bankGuaranteeByEgpId, _bgVendorGid, abi.encodePacked(keyListLength));
        if (_toExcluded > length) {
            _toExcluded = length;
        }
        uint itr = _count>_toExcluded ? _toExcluded:_count;
        _bankGuaranteeByEgpId = new uint[](itr);
        while (itr>0) {
            itr--;
            _toExcluded--;
            _bankGuaranteeByEgpId[_bankGuaranteeByEgpId.length-itr-1] = Map.getByKeyBytes8ToUint(KEY, bankGuaranteeByEgpId, _bgVendorGid, abi.encodePacked(_toExcluded));
        }
        fromIncluded_ = _toExcluded;
        length_ = length;
    }

    function getBankGuaranteeByPeGid(bytes8 _bgPEGid,  uint _toExcluded, uint _count) public view 
    returns(uint[] memory _bankGuaranteeByEgpId, uint fromIncluded_, uint length_)  {
        //bytes8 _bgVendorBankGid = bytes8(keccak256(abi.encodePacked(_vendorGid, _bankGid)));
        uint length = Map.getByKeyBytes8ToUint(KEY, bankGuaranteeByPeGid, _bgPEGid, abi.encodePacked(keyListLength));
        if (_toExcluded > length) {
            _toExcluded = length;
        }
        uint itr = _count>_toExcluded ? _toExcluded:_count;
        ...
        while (itr>0) {
            itr--;
            _toExcluded--;
            _bankGuaranteeByEgpId[_bankGuaranteeByEgpId.length-itr-1] = Map.getByKeyBytes8ToUint(KEY, bankGuaranteeByEgpId, _bgPEGid, abi.encodePacked(_toExcluded));
        }
        fromIncluded_ = _toExcluded;
        length_ = length;
    }
    ...
    /** @dev Listing out the Bank Guarantees against the PE Gid and Bank Gid.
      ...
      * @param _toExcluded The length till where the uint indexes has to be fetched.
      * @param _count The number of indexes that has to be fetched. 
      * @return _bankGuaranteeIndexByPEAndBankGid List of Bank Guarantees against the PE Gid and Bank Gid.
      * @return fromIncluded_ _toExcluded - _count.
      * @return length_ The length of the index present for the PE Gid and Bank Gid.
      */
    function getBankGuaranteeByPEAndBankGid(bytes8 _bgPeBankGid,  uint _toExcluded, uint _count) public view 
    returns(uint[] memory _bankGuaranteeIndexByPEAndBankGid, uint fromIncluded_, uint length_)  {
        ...
        uint itr = _count>_toExcluded ? _toExcluded:_count;
        _bankGuaranteeIndexByPEAndBankGid = new uint[](itr);
        while (itr>0) {
            itr--;
            _toExcluded--;
            _bankGuaranteeIndexByPEAndBankGid[_bankGuaranteeIndexByPEAndBankGid.length-itr-1] = Map.getByKeyBytes8ToUint(KEY, bankGuaranteeByPEAndBankGid, _bgPeBankGid, abi.encodePacked(_toExcluded));
        }
        fromIncluded_ = _toExcluded;
        length_ = length;
    }
    ...
    validBankGuaranteeListIndex(_index) {
        ...
        List.setPropUint(KEY, bankGuaranteeList, _index, propNameBankGuaranteeAmendment,  getBankGuaranteeIndexListLengthByPaymentAdviceAndBankGid(bytes8(keccak256(abi.encodePacked(_BankGuarantee.paymentAdviceNumber,_BankGuarantee.bankGid)))));
    ...
    /** @dev Setting the Bank Guarantees against  Vendor Gid and Bank Gid , Setting the Bank Guarantees against the PE Gid and Bank Gid.
      ...
      */
    function setBankGuaranteeIndices(BankGuarantee memory _bankGuarantee, uint _value, string memory _vendorGid, string memory _peGid, string memory _egpSystemId) public 
    onlyBankGuaranteeManager
    {
        _bankGuaranteeByBankGid(_bankGuarantee.bankGid, _value);
        _bankGuaranteeByEgpSystemId(_egpSystemId, _value);
        _bankGuaranteeByPeGid(_peGid, _value);
        _bankGuaranteeByPaymentAdviceAndBankGid(_bankGuarantee.paymentAdviceNumber,_bankGuarantee.bankGid, _value);
        _bankGuaranteeByVendorBankGid(_vendorGid, _bankGuarantee.bankGid, _value);
        _bankGuaranteeByPEAndBankGid(_peGid, _bankGuarantee.bankGid, _value);
        _bankGuaranteeByRefBankGid(_bankGuarantee.referenceNumber, _bankGuarantee.bankGid, _value);
        _bankGuaranteeByVendorGid(_vendorGid, _value);
        _bankGuaranteeByPaymentAdviceRefAndEgpId(_bankGuarantee.paymentAdviceNumber, _egpSystemId, _value);    
    }
    
    function _bankGuaranteeByBankGid (string memory _bankGid, uint _value) internal {
        bytes8 key = bytes8(keccak256(abi.encodePacked(_bankGid)));
        uint _length = Map.getByKeyBytes8ToUint(KEY, bankGuaranteeByBankGid, key, abi.encodePacked(keyListLength));
        Map.setByKeyBytes8ToUint(KEY, bankGuaranteeByBankGid, key, abi.encodePacked(keyListLength), 1 + _length);
        Map.setByKeyBytes8ToUint(KEY, bankGuaranteeByBankGid, key, abi.encodePacked(_length), _value);
    }

    function _bankGuaranteeByEgpSystemId (string memory _egpSystemId, uint _value) internal {
        bytes8 key = bytes8(keccak256(abi.encodePacked(_egpSystemId)));
        uint _length = Map.getByKeyBytes8ToUint(KEY, bankGuaranteeByEgpId, key, abi.encodePacked(keyListLength));
        Map.setByKeyBytes8ToUint(KEY, bankGuaranteeByEgpId, key, abi.encodePacked(keyListLength), 1 + _length);
        Map.setByKeyBytes8ToUint(KEY, bankGuaranteeByEgpId, key, abi.encodePacked(_length), _value);
    }

    function _bankGuaranteeByPeGid (string memory _peGid, uint _value) internal {
        bytes8 key = bytes8(keccak256(abi.encodePacked(_peGid)));
        uint _length = Map.getByKeyBytes8ToUint(KEY, bankGuaranteeByPeGid, key, abi.encodePacked(keyListLength));
        Map.setByKeyBytes8ToUint(KEY, bankGuaranteeByPeGid, key, abi.encodePacked(keyListLength), 1 + _length);
        Map.setByKeyBytes8ToUint(KEY, bankGuaranteeByPeGid, key, abi.encodePacked(_length), _value);
    }

    function _bankGuaranteeByPaymentAdviceAndBankGid(string memory _paymentAdviceNumber, string memory _bankGid, uint _value) internal {
        bytes8 _uid = bytes8(keccak256(abi.encodePacked(_paymentAdviceNumber,_bankGid)));
        uint _length = Map.getByKeyBytes8ToUint(KEY, bankGuaranteeIndexListByPaymentAdviceAndBankGid, _uid, abi.encodePacked(keyListLength));
        Map.setByKeyBytes8ToUint(KEY, bankGuaranteeIndexListByPaymentAdviceAndBankGid, _uid, abi.encodePacked(_length), _value);
        Map.setByKeyBytes8ToUint(KEY, bankGuaranteeIndexListByPaymentAdviceAndBankGid, _uid, abi.encodePacked(keyListLength), 1+_length);
    }

    function _bankGuaranteeByVendorBankGid(string memory _vendorGid, string memory _bankGid, uint _value) internal {
       bytes8 _bgVendorBankGid = bytes8(keccak256(abi.encodePacked(_vendorGid, _bankGid)));
        uint _length = Map.getByKeyBytes8ToUint(KEY, bankGuaranteeByVendorAndBankGid, _bgVendorBankGid, abi.encodePacked(keyListLength));
        Map.setByKeyBytes8ToUint(KEY, bankGuaranteeByVendorAndBankGid, _bgVendorBankGid, abi.encodePacked(keyListLength), 1+_length);
        Map.setByKeyBytes8ToUint(KEY, bankGuaranteeByVendorAndBankGid, _bgVendorBankGid, abi.encodePacked(_length), _value);
    }

    function _bankGuaranteeByPEAndBankGid(string memory _peGid, string memory _bankGid, uint _value) internal {
        bytes8 _bgPeBankGid = bytes8(keccak256(abi.encodePacked(_peGid, _bankGid)));
        uint _length = Map.getByKeyBytes8ToUint(KEY, bankGuaranteeByPEAndBankGid, _bgPeBankGid, abi.encodePacked(keyListLength));
        Map.setByKeyBytes8ToUint(KEY, bankGuaranteeByPEAndBankGid, _bgPeBankGid, abi.encodePacked(keyListLength), 1+_length);
        Map.setByKeyBytes8ToUint(KEY, bankGuaranteeByPEAndBankGid, _bgPeBankGid, abi.encodePacked(_length), _value);
    }

    function _bankGuaranteeByRefBankGid(string memory _referenceNumber, string memory _bankGid, uint _value) internal {
        bytes8 _bgRefBankGid = bytes8(keccak256(abi.encodePacked(_referenceNumber, _bankGid)));
        uint _length = Map.getByKeyBytes8ToUint(KEY, bankGuaranteeByBgReferenceAndBankGid, _bgRefBankGid, abi.encodePacked(keyListLength));
        Map.setByKeyBytes8ToUint(KEY, bankGuaranteeByBgReferenceAndBankGid, _bgRefBankGid, abi.encodePacked(keyListLength), 1+_length);
        Map.setByKeyBytes8ToUint(KEY, bankGuaranteeByBgReferenceAndBankGid, _bgRefBankGid, abi.encodePacked(_length), _value);
    }

    function _bankGuaranteeByVendorGid(string memory _vendorGid, uint _value) internal {
        bytes8 _bgVendorGid = bytes8(keccak256(abi.encodePacked(_vendorGid)));
        uint _length = Map.getByKeyBytes8ToUint(KEY, bankGuaranteeByVendorGid, _bgVendorGid, abi.encodePacked(keyListLength));
        Map.setByKeyBytes8ToUint(KEY, bankGuaranteeByVendorGid, _bgVendorGid, abi.encodePacked(keyListLength), 1+_length);
        Map.setByKeyBytes8ToUint(KEY, bankGuaranteeByVendorGid, _bgVendorGid, abi.encodePacked(_length), _value);
    }

    function _bankGuaranteeByPaymentAdviceRefAndEgpId(string memory _paymentAdviceNumber, string memory _bankGid, uint _value) internal {
        bytes8 _bgPaRefEgpId = bytes8(keccak256(abi.encodePacked(_paymentAdviceNumber,_bankGid)));
        uint _length = Map.getByKeyBytes8ToUint(KEY, bankGuaranteeIndexListByPaymentAdviceRefAndEgpId, _bgPaRefEgpId, abi.encodePacked(keyListLength));
        Map.setByKeyBytes8ToUint(KEY, bankGuaranteeIndexListByPaymentAdviceRefAndEgpId, _bgPaRefEgpId, abi.encodePacked(_length), _value);
        Map.setByKeyBytes8ToUint(KEY, bankGuaranteeIndexListByPaymentAdviceRefAndEgpId, _bgPaRefEgpId, abi.encodePacked(keyListLength), 1+_length);
    }
