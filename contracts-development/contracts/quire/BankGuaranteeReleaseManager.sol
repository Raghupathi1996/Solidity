...
contract BankGuaranteeReleaseManager is BankGuaranteeReleaseStructs, BankGuaranteeInvokeStructs {
    ...
    returns(BankGuaranteeRelease[] memory _bgReleaseListByBankGID, uint fromIncluded_, uint length_)  {
        ...
        (indexList, fromIncluded_, length_) = bankGuaranteeReleaseModel.getBgReleaseListByBankGID(_bankGid, _toExcluded, _count);
    ...
    returns(BankGuaranteeInvoke[] memory _bgInvokeListByBankGID, uint fromIncluded_, uint length_)  {
        uint[] memory indexList;
        (indexList, fromIncluded_, length_) = bankGuaranteeInvokeModel.getBgInvokeListByBankGID(_bankGid, _toExcluded, _count);
    ...
    returns(BankGuaranteeRelease[] memory _bgReleaseListByUid, BankGuaranteeInvoke[] memory _bgInvokeListByUid, uint fromIncluded_, uint length_)  {
        ...
        uint[] memory indexList;
        uint[] memory mapList;
        (indexList, mapList, fromIncluded_, length_) = bankGuaranteeReleaseModel.getBgReleaseAndInvokeListByBgUid(_bgReleaseInvokePaRefBankGid, _toExcluded, _count);
    ...
    // function getBgReleasesAndInvokesListUidsByPaBankGid(string memory _paReferenceNumber, string memory _bankGid,  uint _toExcluded, uint _count) public view 
    // returns(BankGuaranteeRelease[] memory _bgReleaseListByUid, BankGuaranteeInvoke[] memory _bgInvokeListByUid, bytes8[] memory indexList, uint fromIncluded_, uint length_)  {
    //     bytes8 _bgReleaseInvokePaRefBankGid = bytes8(keccak256(abi.encodePacked(_paReferenceNumber, _bankGid)));
    //     // bytes8[] memory indexList;
    //     uint[] memory mapList;
    //     uint i =0;
    //     (indexList, mapList, fromIncluded_, length_) = bankGuaranteeReleaseModel.getBgReleaseAndInvokeUidsByBgUid(_bgReleaseInvokePaRefBankGid, _toExcluded, _count);
    //     uint _length = _count;
    //     _bgReleaseListByUid = new BankGuaranteeRelease [](indexList.length);
    //     _bgInvokeListByUid = new BankGuaranteeInvoke [](indexList.length);
    //     while( i < indexList.length && _length > 0) {
    //         if(mapList[i] == 1)
    //         {
    //             bytes8 uid = indexList[i];
    //             _bgInvokeListByUid[i] = getBankGuaranteeInvoke(uid);
    //             i++;
    //             _length--;
    //         } else {
    //             bytes8 uid = indexList[i];
    //             _bgReleaseListByUid[i] = getBankGuaranteeRelease(uid);
    //             i++;
    //             _length--;
    //         }
    //     }
    // }
    // //Test
    // function getBgReleaseAndInvokeListByBgUidM (string memory _paReferenceNumber, string memory _bankGid,  uint _toExcluded, uint _count) public view
    // returns(uint256[] memory indexList, uint fromIncluded_, uint length_, uint256[] memory mapReport)  {
    //     bytes8 _bgReleaseInvokePaRefBankGid = bytes8(keccak256(abi.encodePacked(_paReferenceNumber, _bankGid)));
    ...
    //     mapReport = new uint256[] (indexList.length);
    //     for (uint256 i = 0; i < indexList.length; i++) {
    //         mapReport[i] = bankGuaranteeReleaseModel.getReleaseInvokeStatus(_bgReleaseInvokePaRefBankGid, i);
    //     }
    // }

    // function getBgReleaseAndInvokeListByBgUidMo (string memory _paReferenceNumber, string memory _bankGid,  uint _toExcluded, uint _count) public view
    // returns(BankGuaranteeRelease[] memory _bgReleaseListByUid, BankGuaranteeInvoke[] memory _bgInvokeListByUid)  {
    //     bytes8 _bgReleaseInvokePaRefBankGid = bytes8(keccak256(abi.encodePacked(_paReferenceNumber, _bankGid)));
    //     // (indexList, fromIncluded_, length_) = bankGuaranteeReleaseModel.getBgReleaseAndInvokeListByBgUid(_bgReleaseInvokePaRefBankGid, _toExcluded, _count);
    //     uint j = 0;
    //     uint mapR;
    //     _bgReleaseListByUid = new BankGuaranteeRelease [](_count);
    //     _bgInvokeListByUid = new BankGuaranteeInvoke [](_count);
    //     // mapReport = new uint256[] (_count-_toExcluded);
    //     // mapR = new uint256[] (_count-_toExcluded);

    //     // for (uint256 i = _toExcluded; i < _count; i++) {
    //     //     mapR[j] = bankGuaranteeReleaseModel.getReleaseInvokeStatus(_bgReleaseInvokePaRefBankGid, i);
    //     //     j++;
    //     // }
    //     // for (uint256 i = _toExcluded; i < _count; i++) {
    //     while (_count > 0) {
    //         mapR = bankGuaranteeReleaseModel.getReleaseInvokeStatus(_bgReleaseInvokePaRefBankGid, _toExcluded);
    //         if(mapR == 1) {
    //             _bgInvokeListByUid[j] = bankGuaranteeInvokeModel.getBankGuaranteeInvokeByIndex(_toExcluded);
    //             j++;
    //             _count--;
    //             _toExcluded--;
    //         } else {
    //             _bgReleaseListByUid[j] = bankGuaranteeReleaseModel.getBankGuaranteeReleaseByIndex(_toExcluded);
    //             j++;
    //             _count--;
    //             _toExcluded--;
    //         }
