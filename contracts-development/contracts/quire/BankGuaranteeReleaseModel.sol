...
contract BankGuaranteeReleaseModel is BankGuaranteeReleaseStructs {
    ...
    string constant internal bgReleaseAndInvokeListByBgUid = "list__bgReleaseAndInvokeListByBgUid";
    ...
    modifier onlyBankGuaranteeInvokeModel() {
        require(msg.sender==quireContracts.getRegisteredContract("BANK_GUARANTEE_INVOKE_MODEL") || msg.sender==quireContracts.getRegisteredContract(KEY), "Unauthorized Contract Call");
        _;
    }
    ...
    function setBankGuaranteeReleaseIndex(bytes8 _uid, uint256 _value) public 
    ...
    /** @dev Setting the Bank Guarantee Releases against the Bank Gid, Setting the Bank Guarantee Releases against the EgpSystem Id , Setting the Bank Guarantee Releases against the Bank Guarantee Reference Number and Bank Gid, Setting the Bank Guarantee Releases against the Payment Advice Reference Number and Bank Gid.
      ...
      */
    function setBankGuaranteeReleaseIndices(BankGuaranteeRelease memory _BankGuaranteeRelease, uint256 _value, string memory _bgReferenceNumber) public 
    onlyBankGuaranteeReleaseManager
    {
        ...
        setBgReleaseAndInvokeListByBgUid(_bgReleasePaRefBankGid, _value, 0);
    }
    ...
    /** @dev Listing out the Bank Guarantee Releases and Invokes against the BG Uid.
      ...
      * @return _bgReleaseAndInvokeMapByBgUid Map List of Bank Guarantee Releases and Invokes against the BG Uid.
      * @return fromIncluded_ _toExcluded - _count.
    ...
    function getBgReleaseAndInvokeListByBgUid(bytes8 _bgUid,  uint _toExcluded, uint _count) public view 
    returns(uint256[] memory _bgReleaseAndInvokeListByBgUid, uint[] memory _bgReleaseAndInvokeMapByBgUid, uint fromIncluded_, uint length_)  {
        ...
        _bgReleaseAndInvokeMapByBgUid = new uint[](itr);
        while (itr>0) {
            itr--;
            _toExcluded--;
            ...
            _bgReleaseAndInvokeMapByBgUid[_bgReleaseAndInvokeListByBgUid.length-itr-1] = Map.getByKeyBytes8ToUint(KEY, mapBgReleaseAndInvokeByUid, _bgUid, abi.encodePacked(_toExcluded));
    ...
    function getBgReleaseAndInvokeUidsByBgUid(bytes8 _bgUid,  uint _toExcluded, uint _count) public view 
    returns(bytes8[] memory _bgReleaseAndInvokeListByBgUid, uint[] memory _bgReleaseAndInvokeMapByBgUid, uint fromIncluded_, uint length_)  {
        uint length = Map.getByKeyBytes8ToUint(KEY, bgReleaseAndInvokeListByBgUid, _bgUid, abi.encodePacked(keyListLength));
        if (_toExcluded > length) {
            _toExcluded = length;
        }
        uint itr = _count>_toExcluded ? _toExcluded:_count;
        _bgReleaseAndInvokeListByBgUid = new bytes8[](itr);
        _bgReleaseAndInvokeMapByBgUid = new uint[](itr);
        while (itr>0) {
            itr--;
            _toExcluded--;
            _bgReleaseAndInvokeListByBgUid[_bgReleaseAndInvokeListByBgUid.length-itr-1] = Map.getByKeyBytes8ToBytes8(KEY, bgReleaseAndInvokeListByBgUid, _bgUid, abi.encodePacked(_toExcluded));
            _bgReleaseAndInvokeMapByBgUid[_bgReleaseAndInvokeListByBgUid.length-itr-1] = Map.getByKeyBytes8ToUint(KEY, mapBgReleaseAndInvokeByUid, _bgUid, abi.encodePacked(_toExcluded));
        }
        fromIncluded_ = _toExcluded;
        length_ = length;
    ...
    // function getBgReleaseAndInvokeUidByBgUid(bytes8 _bgUid,  uint _toExcluded, uint _count) public view returns(uint256[] memory _bgReleaseAndInvokeListByBgUid, uint fromIncluded_, uint length_)  {
    //     uint length = Map.getByKeyBytes8ToUint(KEY, bgReleaseAndInvokeListByBgUid, _bgUid, abi.encodePacked(keyListLength));
    //     if (_toExcluded > length) {
    //         _toExcluded = length;
    //     }
    ...
    /** @dev Setting the Bank Guarantee Releases and Bank Guarantee Invokes against the Bank Guarantee Uid.
      ...
      */
    function setBgReleaseAndInvokeListByBgUid(bytes8 _Uid, uint _value, uint _flag) public
    onlyBankGuaranteeInvokeModel
    {
        // set bgReleaseAndInvokeListByBgUid
        uint256 length = Map.getByKeyBytes8ToUint(KEY, bgReleaseAndInvokeListByBgUid, _Uid, abi.encodePacked(keyListLength));
        ...
        Map.setByKeyBytes8ToUint(KEY, mapBgReleaseAndInvokeByUid, _Uid, abi.encodePacked(length), _flag);
    }
