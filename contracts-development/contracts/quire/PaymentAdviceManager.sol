...
contract PaymentAdviceManager is PaymentAdviceStructs {
    ...
    validCurrency(_paymentAdvice.currency) {
        ...
        _paymentAdvice.amendment = paymentAdviceModel.getPaymentAdviceIndexListLengthByPaymentAdviceAndEgpId(bytes8(keccak256(abi.encodePacked(_paymentAdvice.paymentAdviceReferenceNumber,_paymentAdvice.egpSystemId)))) -1;
    ...
    function getPaymentAdviceListByPaymentAdviceRefAndEgpIdTest(string memory _paymentAdviceReferenceNumber, string memory _egpSystemId, uint _toExcluded, uint _count) public view 
    returns (uint[] memory indexList, uint fromIncluded_, uint length_) {
        bytes8 uid = bytes8(keccak256(abi.encodePacked(_paymentAdviceReferenceNumber,_egpSystemId)));
        (indexList, fromIncluded_, length_) = paymentAdviceModel.getPaymentAdviceIndexListByPaymentAdviceAndEgpId(uid, _toExcluded, _count);
    }
    ...
    function getPaymentAdviceProcuringEntityGidbyUid (bytes8 _uid) public view
    paymentAdviceExists(_uid) 
    returns (string memory _gid)
    {
        uint256 _index = paymentAdviceModel.getPaymentAdviceIndex(_uid) - 1;
    ...
    function getPaymentAdviceEgpbyUid (bytes8 _uid) public view
    paymentAdviceExists(_uid) 
    returns (string memory _gid)
    {
        uint256 _index = paymentAdviceModel.getPaymentAdviceIndex(_uid) - 1;
        return paymentAdviceModel.getPaymentAdviceEgpSystemId(_index); 
    }
