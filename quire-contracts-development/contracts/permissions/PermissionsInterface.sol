...
contract PermissionsInterface {
    ...
    /** @notice interface to add sub org under an org, create a new role for this org,
      ...
      */
    function addSubOrgWithExtDetails(string memory _pOrgId, string memory _orgId, 
        bytes memory _nodeDetails, bytes memory _roleDetails, 
        bytes memory _orgExtDetails, address _account, string memory _accountName) public {
      permImplementation.addSubOrgWithAccountAndRole(_pOrgId, _orgId, _nodeDetails, _roleDetails, _orgExtDetails, _account, _accountName, msg.sender);
    }
    ...
    function updateOrgExternalDetails(string memory _orgId, string memory _name, string memory _location, string memory _url) public {
        permImplementation.updateOrgExternalDetails(_orgId, _name, _location, _url, msg.sender);
    }
    
    function updateOrgUrl(string memory _orgId, string memory _url) public {
        permImplementation.updateOrgUrl(_orgId, _url, msg.sender);
    }
    ...
    function assignAccountName(address _account, string calldata _name) external {
        permImplementation.assignAccountName(_account, _name, msg.sender);

    }

    /** @notice interface to assigns a role id and name to the account given
      * @param _account account id
      * @param _orgId organization id to which the account belongs
      * @param _roleId role id to be assigned to the account
      * @param _name name to be assigned to the account
      */
    function assignAccountRoleAndName(address _account, string calldata _orgId,
        string calldata _roleId, string calldata _name) external {
      permImplementation.assignAccountRoleAndName(_account, _orgId, _roleId, _name, msg.sender);
    }
    ...
    function validateOrgAndAccount(address _account, string calldata _orgId)
    external view returns (bool) {
        return permImplementation.validateOrgAndAccount(_account, _orgId);
    }
